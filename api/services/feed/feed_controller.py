...
class FeedController:
    ...
    async def cache_feeds_for_users(
        self, tail_id: str, limit: int = 20, skip: int = 0
    ) -> None:
        async with self.manager.client() as session:
            session: AsyncSession
            users = list(
                (
                    await session.execute(
                        select(Subscription.head_id).where(
