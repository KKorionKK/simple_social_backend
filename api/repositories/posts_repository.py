...
class PostsRepository(Repository):
    ...
    async def get_likes_comments_count(self, post_id: str) -> tuple[int, int]:
        async with self.client() as session:
            session: AsyncSession
            likes = len((await session.execute(select(Like).where(and_(Like.post_id==post_id, Like.comment_id == None)))).scalars().fetchall())  # noqa: E711
            comments = len((await session.execute(select(Comment).where(Comment.post_id==post_id))).scalars().fetchall())
    ...
    async def modify_post(self, post_id: str, payload: CreatePostSchema, user_id: str) -> PostSchemaExtended:
        async with self.client() as session:
            session: AsyncSession
            await session.execute(
                update(Post)
