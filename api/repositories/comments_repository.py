...
class CommentsRepository(Repository):
        ...
        async with self.client() as session:
            ...
            await session.execute(
                delete(Comment).where(
                    and_(Comment.user_id == user_id, Comment.id == comment_id)
                )
            )
            await session.commit()

    async def modify_comment(
        self, payload: CommentUpdateSchema, user_id: str
    ) -> Comment:
        async with self.client() as session:
            session: AsyncSession
            await session.execute(
                update(Comment)
                .where(
                    and_(Comment.id == payload.comment_id, Comment.user_id == user_id)
                )
    ...
    async def create_comment(
        self, user_id: str, payload: CreateCommentSchema
    ) -> PostCommentsSchema:
        ...
        return PostCommentsSchema(comments=[item.as_schema() for item in comments])

    async def list_post_comments(
        self, post_id: str, limit: int = 10, skip: int = 0
    ) -> list[Comment]:
        async with self.client() as session:
            session: AsyncSession
            result = (
    ...
    async def list_thread_comments(
        self, thread_id: str, limit: int = 10, skip: int = 10
    ) -> list[Comment]:
        async with self.client() as session:
            session: AsyncSession
            result = (
    ...
    async def add_comment_like(self, payload: CommentLikeSchema, user_id: str) -> Like:
        ...
        model = Like(
            comment_id=payload.comment_id, post_id=payload.post_id, user_id=user_id
        )
        async with self.client() as session:
            session: AsyncSession
            session.add(model)
    ...
    async def delete_comment_like(self, comment_id: str, user_id: str):
        async with self.client() as session:
            ...
            await session.execute(
                delete(Like).where(
                    and_(Like.comment_id == comment_id, Like.user_id == user_id)
                )
            )
