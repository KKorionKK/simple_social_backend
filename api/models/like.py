...
class Like(Base):
    ...
    created_at: Mapped[DateTime] = mapped_column(DateTime(timezone=True), default=tools.get_dt, nullable=False)

    user_id: Mapped[String] = mapped_column(String, ForeignKey('users.id'), nullable=False)
    post_id: Mapped[String] = mapped_column(String, ForeignKey('posts.id', ondelete='CASCADE'), nullable=False)
    comment_id: Mapped[String] = mapped_column(String, ForeignKey('comments.id', ondelete='CASCADE'), nullable=True)
